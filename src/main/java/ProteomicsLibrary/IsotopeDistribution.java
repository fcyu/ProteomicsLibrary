...
public class IsotopeDistribution {
    ...
    public IsotopeDistribution(Map<String, Double> elementTable, double limit, String labelling) {
    ...
    public Map<String, Integer> getElementMapFromMonoMass(double mass) {
    ...
    public Entry getIsotopeCorrectionNum(double precursorMz, double ms1Tolerance, int ms1ToleranceUnit, int charge, TreeMap<Double, Double> parentPeakList) throws Exception {
        ...
        for (int isotopeCorrectionNum : isotopeCorrectionArray) {
            ...
            if (Math.abs(expMatrix[0][1]) > 1) { // In bottom-up proteomics, the precursor mass won't be so large that we cannot observe the monoisotopic peak.
                Map<String, Integer> elementMap = getElementMapFromMonoMass((expMatrix[0][0] - MassTool.PROTON) * charge);
                List<IsotopeDistribution.Peak> theoIsotopeDistribution = calculateIsotopePeaks(elementMap);
                int peakNum = Math.min(expMatrix.length, theoIsotopeDistribution.size());
                double[] expArray = new double[peakNum];
                double[] theoArray = new double[peakNum];
                for (int i = 0; i < peakNum; ++i) {
                    expArray[i] = expMatrix[i][1];
                    IsotopeDistribution.Peak peak = theoIsotopeDistribution.get(i);
                    theoArray[i] = peak.realArea;
                }
                double pearsonCorrelationCoefficient = Statistics.calPearsonCorrelationCoefficient(theoArray, expArray);
    ...
    public class Entry {
        ...
        public int isotopeCorrectionNum;
